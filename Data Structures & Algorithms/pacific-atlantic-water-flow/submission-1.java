class Solution {
    ...
    public void dfs(int[][] heights, int row, int col, boolean[][] visited, int prev){
        if(row < 0 || row >= heights.length || col < 0 || col >= heights[0].length || heights[row][col] < prev || visited[row][col] == true){
            return;
        }
